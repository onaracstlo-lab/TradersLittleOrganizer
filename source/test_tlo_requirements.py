...
def test_v305_tagger_gui_keeps_bold_app_heading_and_uses_current_public_version():
    ...
    assert gui.TAGGER_DISPLAY_VERSION == "TLO Tagger GUI v1.0 Build 318"
...
def test_v305_public_version_matches_bundle_number():
    ...
    assert V.VERSION == "v318"
    assert V.BUNDLE_BUILD == 318
    assert V.DISPLAY_VERSION == "v1.0 Build 318"
    assert V.versioned_title("TLO Inventory GUI") == "TLO Inventory GUI v1.0 Build 318"
...
def test_v305_startup_banner_never_appends_release_change_summary():
    ...
    for debug in (False, True):
        ...
        assert banner == "Starting tlo-gi v1.0 Build 318"
...
def test_v305_all_toplevel_gui_titles_include_public_version():
    ...
    assert gui.WINDOW_TITLE == "TLO Inventory GUI v1.0 Build 318"
    assert gui.TAGGER_DISPLAY_VERSION == "TLO Tagger GUI v1.0 Build 318"
    assert UPDATER_DISPLAY_VERSION == "TLO Inventory Updater v1.0 Build 318"
...
# --------------------------------------------------------------------------- #
# v318 - CorruptFlacs.txt and tlo-gsi --myTLO compatibility
# --------------------------------------------------------------------------- #
...
def test_v318_tlo_gsi_accepts_hidden_mytlo_with_same_precedence(tmp_path, monkeypatch):
...
def test_v318_corrupt_flacs_log_is_created_and_records_only_flac_paths(tmp_path):
    from inventory_parser_lib import Config
...
def test_v318_flac_tag_write_failures_are_recorded_to_corrupt_flacs(tmp_path, monkeypatch):
    import tlo_tag_lib as taglib
