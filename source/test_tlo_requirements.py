...
def test_v305_tagger_gui_keeps_bold_app_heading_and_uses_current_public_version():
    ...
    assert gui.TAGGER_DISPLAY_VERSION == "TLO Tagger GUI v1.0 Build 321"
...
def test_v305_public_version_matches_bundle_number():
    ...
    assert V.VERSION == "v321"
    assert V.BUNDLE_BUILD == 321
    assert V.DISPLAY_VERSION == "v1.0 Build 321"
    assert V.versioned_title("TLO Inventory GUI") == "TLO Inventory GUI v1.0 Build 321"
...
def test_v305_startup_banner_never_appends_release_change_summary():
    ...
    for debug in (False, True):
        ...
        assert banner == "Starting tlo-gi v1.0 Build 321"
...
def test_v305_all_toplevel_gui_titles_include_public_version():
    ...
    assert gui.WINDOW_TITLE == "TLO Inventory GUI v1.0 Build 321"
    assert gui.TAGGER_DISPLAY_VERSION == "TLO Tagger GUI v1.0 Build 321"
    assert UPDATER_DISPLAY_VERSION == "TLO Inventory Updater v1.0 Build 321"
...
# v321 - Packaged platform icon assets

def test_v321_packaged_platform_icon_assets_are_present():
    from pathlib import Path
...
def test_v321_windows_dist_uses_packaged_ico_files_directly():
    from pathlib import Path
...
def test_v321_windows_ico_assets_are_dib_based_not_png_compressed():
    import struct
    from pathlib import Path
...
def test_v321_windows_dist_verifies_exact_packaged_icon_resources():
    from pathlib import Path
    text = (Path(__file__).resolve().parent / "createWindowsDist.ps1").read_text(encoding="utf-8")
