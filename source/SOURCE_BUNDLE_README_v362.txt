TLO Source and Utilities Bundle v362

Public application version: v1.3 Build 362
Source bundle label: v362

Build 362 summary
- Classifies folder/show name collisions as copies only after exact recursive tree and byte-content comparison.
- Uses (copyN) only for identical directory structure, relative files, sizes, and file bytes; otherwise uses (altN).
- Revalidates pre-existing (copyN) source folders during metadata extraction and exposes them as alt when no exact same-base sibling exists.
- Uses SHA-256 content comparison after structure/path/size checks; timestamps do not determine copy identity.
...
- TLO_Inventory_User_Manual_v362.rtf: current end-user manual.
- TLO_Inventory_Requirements_Working_v362.docx: current requirements/development document.
- TLO-FAQ.txt: current frequently asked questions.
- CHANGES_v362.txt: release changes and preserved behavior.
- old-change-logs.zip: archived historical change notes through Build 361.

Test layout:
- pytest.ini: registered markers and categorized discovery configuration.
- test_tlo_requirements.py: Build Process v055 compatibility entry point.
...
Recommended build process bundle: TLO_GitHub_Build_Process_v055.zip.
...
- SHA256SUMS_v362.txt records hashes for the files in this source bundle.

Validation status:
- 579 tests pass with GUI display support.
- 577 tests pass and 2 GUI tests skip explicitly without a display using the Build Process v055 compatibility entry point.
