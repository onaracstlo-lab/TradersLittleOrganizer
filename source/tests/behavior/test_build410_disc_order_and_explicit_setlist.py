...
def test_build410_requirements_and_manual_document_reported_fix():
    ...
    requirements = Document(root / "TLO_Inventory_Requirements_Working_v461.docx")
    req_text = "\n".join(p.text for p in requirements.paragraphs)
    manual_text = (root / "TLO_Inventory_User_Manual_v461.rtf").read_text(encoding="utf-8", errors="ignore")

    assert "Current document version: v461 (v1.6 Build 461)." in req_text
    ...
    assert "Version v1.6 Build 461" in manual_text
