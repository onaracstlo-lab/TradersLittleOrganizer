...
def test_build476_single_drop_with_multiple_folders_inserts_semicolons(tmp_path, monkeypatch):
    monkeypatch.setattr(DD, "is_drag_drop_platform", lambda: True)
...
def test_build476_dropped_path_with_comma_needs_no_quotes(tmp_path, monkeypatch):
    monkeypatch.setattr(DD, "is_drag_drop_platform", lambda: True)
...
def test_build462_documentation_records_cumulative_multidrop_behavior():
    ...
    req = "\n".join(p.text for p in Document(root / "TLO_Inventory_Requirements_Working_v478.docx").paragraphs)
    manual = (root / "TLO_Inventory_User_Manual_v478.rtf").read_text(encoding="utf-8", errors="ignore")
    faq = (root / "TLO-FAQ.txt").read_text(encoding="utf-8")

    assert "Current document version: v478 (v1.7 Build 478)." in req
