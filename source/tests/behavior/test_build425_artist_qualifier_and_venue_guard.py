...
def test_build425_requirements_and_manual_document_rule():
    ...
    requirements = Document(root / "TLO_Inventory_Requirements_Working_v453.docx")
    requirements_text = "\n".join(paragraph.text for paragraph in requirements.paragraphs)
    manual_text = (root / "TLO_Inventory_User_Manual_v453.rtf").read_text(
        encoding="utf-8", errors="ignore"
    )

    assert "Current document version: v453 (v1.6 Build 453)." in requirements_text
    ...
    assert "Version v1.6 Build 453" in manual_text
