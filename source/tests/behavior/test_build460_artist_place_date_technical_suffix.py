...
def test_build460_requirements_and_manual_document_guarded_fallback():
    ...
    req = "\n".join(
        paragraph.text for paragraph in Document(root / "TLO_Inventory_Requirements_Working_v465.docx").paragraphs
    )
    manual = (root / "TLO_Inventory_User_Manual_v465.rtf").read_text(
        encoding="utf-8", errors="ignore"
    )

    assert "Current document version: v465 (v1.6 Build 465)." in req
    ...
    assert "Version v1.6 Build 465" in manual
