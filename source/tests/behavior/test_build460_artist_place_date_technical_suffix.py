...
def test_build460_requirements_and_manual_document_guarded_fallback():
    ...
    req = "\n".join(
        paragraph.text for paragraph in Document(root / "TLO_Inventory_Requirements_Working_v476.docx").paragraphs
    )
    manual = (root / "TLO_Inventory_User_Manual_v476.rtf").read_text(
        encoding="utf-8", errors="ignore"
    )

    assert "Current document version: v476 (v1.7 Build 476)." in req
    ...
    assert "Version v1.7 Build 476" in manual
