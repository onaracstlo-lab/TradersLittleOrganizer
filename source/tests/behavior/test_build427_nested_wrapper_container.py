...
def test_build427_documentation_contract():
    ...
    requirements = "\n".join(
        paragraph.text for paragraph in Document(root / "TLO_Inventory_Requirements_Working_v490.docx").paragraphs
    )
    manual = (root / "TLO_Inventory_User_Manual_v490.rtf").read_text(encoding="utf-8", errors="ignore")

    assert "Current document version: v490 (v1.7 Build 490)." in requirements
