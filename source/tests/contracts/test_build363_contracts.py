"""Build 363 documentation and release contracts."""
...
def test_build363_requirements_define_commercial_release_year_and_no_duplicate_artist():
    text = _docx_text("TLO_Inventory_Requirements_Working_v363.docx")
...
def test_build363_requirements_define_complete_recursive_transfer_tree():
    text = _docx_text("TLO_Inventory_Requirements_Working_v363.docx")
...
def test_build363_manual_documents_commercial_release_and_complete_tree_copy():
    text = (ROOT / "TLO_Inventory_User_Manual_v363.rtf").read_text(encoding="utf-8", errors="ignore")
...
def test_build363_archives_prior_change_log():
    with ZipFile(ROOT / "old-change-logs.zip") as zf:
        assert "CHANGES_v362.txt" in zf.namelist()


def test_build363_requirements_define_exact_copy_vs_alt_classification():
    text = _docx_text("TLO_Inventory_Requirements_Working_v363.docx")
...
def test_build363_manual_documents_copy_vs_alt_classification():
    text = (ROOT / "TLO_Inventory_User_Manual_v363.rtf").read_text(encoding="utf-8", errors="ignore")
...
def test_build363_delete_dupes_contracts_and_packaging():
    ...
    for text in (windows, linux, mac):
        ...
        assert "send2trash" in text


def test_build363_requirements_document_delete_dupes_behavior():
    text = _docx_text("TLO_Inventory_Requirements_Working_v363.docx")
    assert "tlo-deleteDupes.py" in text
    ...
    assert "Recycle Bin or Trash" in text
    assert "must never move the unsuffixed original directory" in text


def test_build363_manual_documents_delete_dupes_behavior():
    text = (ROOT / "TLO_Inventory_User_Manual_v363.rtf").read_text(encoding="utf-8", errors="ignore")
